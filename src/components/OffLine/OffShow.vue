          ...
          <div slot="header"  class="clearfix">
            ...
            <div class="people">   
                <el-radio-group v-model="radio1" >
                  <el-radio-button label="option1"><div style="width:50px">option1</div></el-radio-button>
                  <el-radio-button label="option2"><div style="width:50px" @click="setting">option2</div></el-radio-button>
                </el-radio-group>
...
export default {
  ...
  methods:{
      ...
      newData(){
        ...
        this.participant.length=this.number
        //使用强制渲染
        this.$forceUpdate();
      },
      getInput(){
      // this.participant.length=this.participant
          ...
          {
            for(var j=0;inputData.substring(j-1,j)!=' ';j++)
            {console.log(inputData.substring(j-1,j)!=' ')}
            //通过空格来取空括前面几位并存入数组中
            this.participant[i]=inputData.substring(0,j);
            console.log(j)
            // 去掉带空格以及前面的数字
